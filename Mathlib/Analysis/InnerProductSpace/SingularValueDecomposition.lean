...
@[expose] noncomputable def svdTerm (i : Fin n) (x : E) : F :=
...
/-! ## Three-way factorization `T = U ∘ₗ Σ ∘ₗ V`

We split the "ΣU" map `D = sigmaU` into the diagonal scaling `Σ` (on the Euclidean domain) and
the left-singular-vector map `U`, giving the classical three-factor form of the SVD.

**Design note.** `diagonalSingularValues` omits `hn` from its parameter list; its body only uses
`T` and `n` (implicit), not `hn : finrank 𝕜 E = n`. This avoids a Lean 4 bidirectional-typing
issue: when the expected return type is `EuclideanSpace 𝕜 (Fin n) →ₗ EuclideanSpace 𝕜 (Fin n)`,
Lean can uniquely pin `n`, and therefore auto-fills any explicit Prop arg `hn : finrank 𝕜 E = n`
from scope—consuming it before the user's `hn` token, causing a type mismatch. Leaving `hn`
out of the def entirely sidesteps this. `leftSingularLinearMap` does use `hn` (calling
`T.leftSingularVector hn i`), but its codomain is `F`, so `n` cannot be pinned from the return
type and the auto-fill does not trigger there. -/

/-- **(Three-way) the diagonal scaling map `Σ`.** The linear endomorphism of
`EuclideanSpace 𝕜 (Fin n)` sending `eᵢ ↦ (σᵢ : 𝕜) • eᵢ`. `T` is drawn from the surrounding
`variable {T hn}` context; `n` is the implicit `{n : ℕ}` variable. `hn` is not a parameter
(see the design note above). -/
noncomputable def diagonalSingularValues :
    EuclideanSpace 𝕜 (Fin n) →ₗ[𝕜] EuclideanSpace 𝕜 (Fin n) :=
  (EuclideanSpace.basisFun (Fin n) 𝕜).toBasis.constr 𝕜
    (fun i => (T.singularValues i : 𝕜) • EuclideanSpace.single i (1 : 𝕜))

/-- **(Three-way) the left-singular-vector map `U`.** The linear map
`EuclideanSpace 𝕜 (Fin n) →ₗ[𝕜] F` sending `eᵢ ↦ T.leftSingularVector hn i`. -/
noncomputable def leftSingularLinearMap (T : E →ₗ[𝕜] F) {n : ℕ} (hn : finrank 𝕜 E = n) :
    EuclideanSpace 𝕜 (Fin n) →ₗ[𝕜] F :=
  (EuclideanSpace.basisFun (Fin n) 𝕜).toBasis.constr 𝕜
    (fun i => T.leftSingularVector hn i)

/-- `Σ` sends `eᵢ` to `(σᵢ : 𝕜) • eᵢ`. -/
theorem diagonalSingularValues_single (i : Fin n) :
    T.diagonalSingularValues (EuclideanSpace.single i (1 : 𝕜))
      = (T.singularValues i : 𝕜) • EuclideanSpace.single i (1 : 𝕜) := by
  have hb : (EuclideanSpace.basisFun (Fin n) 𝕜).toBasis i = EuclideanSpace.single i (1 : 𝕜) := by
    rw [OrthonormalBasis.coe_toBasis, EuclideanSpace.basisFun_apply]
  simp only [diagonalSingularValues, ← hb, Basis.constr_basis]

/-- `U` sends `eᵢ` to `T.leftSingularVector hn i`. -/
theorem leftSingularLinearMap_single (T : E →ₗ[𝕜] F) {n : ℕ} (hn : finrank 𝕜 E = n)
    (i : Fin n) :
    leftSingularLinearMap T hn (EuclideanSpace.single i (1 : 𝕜)) = T.leftSingularVector hn i := by
  have hb : (EuclideanSpace.basisFun (Fin n) 𝕜).toBasis i = EuclideanSpace.single i (1 : 𝕜) := by
    rw [OrthonormalBasis.coe_toBasis, EuclideanSpace.basisFun_apply]
  simp only [leftSingularLinearMap, ← hb, Basis.constr_basis]

/-- **(Three-way) `D = U ∘ₗ Σ`.** The "ΣU" map `sigmaU` splits as
`T.sigmaU hn = leftSingularLinearMap T hn ∘ₗ T.diagonalSingularValues`. -/
theorem sigmaU_eq_comp :
    T.sigmaU hn = leftSingularLinearMap T hn ∘ₗ T.diagonalSingularValues := by
  apply (EuclideanSpace.basisFun (Fin n) 𝕜).toBasis.ext
  intro i
  have hb : (EuclideanSpace.basisFun (Fin n) 𝕜).toBasis i = EuclideanSpace.single i (1 : 𝕜) := by
    rw [OrthonormalBasis.coe_toBasis, EuclideanSpace.basisFun_apply]
  rw [hb, LinearMap.comp_apply,
      diagonalSingularValues_single (T := T) i,
      LinearMap.map_smul,
      leftSingularLinearMap_single T hn i,
      sigmaU_single_eq_smul (T := T) (hn := hn) i]

/-- **(THREE-WAY FACTORIZATION)** The classical three-factor SVD operator equation:
`T = U ∘ₗ (Σ ∘ₗ V)`, where `V = rightIsometry hn`, `Σ = T.diagonalSingularValues`,
`U = leftSingularLinearMap T hn`. Follows from `svd_factorization` and `sigmaU_eq_comp`. -/
theorem svd_three_way_factorization :
    T = leftSingularLinearMap T hn ∘ₗ
        (T.diagonalSingularValues ∘ₗ
          (T.rightIsometry hn).toLinearEquiv.toLinearMap) := by
  rw [← LinearMap.comp_assoc, ← sigmaU_eq_comp (T := T) (hn := hn)]
  exact svd_factorization (T := T) (hn := hn)
